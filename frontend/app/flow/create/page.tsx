        ...
        <div className="relative p-4 w-full max-w-2xl max-h-full">
            <div className="relative bg-white rounded-lg shadow ">
                ...
                <div className="p-4 md:p-5 space-y-4">
                    ...
                    {(step === 1 && selectedAction?.id === "slack") && <SlackSelector setMetadata={(metadata) => {
                        onSelect({
                            ...selectedAction,
                            metadata
                        })
...
function SlackSelector({setMetadata}: {
    setMetadata: (params: any) => void;
}) {
    const [amount, setAmount] = useState("");
    const [address, setAddress] = useState("");    
    const [hookUrl,setUrl]=useState("");
    return <div>
        <Input label={"ChannelWebhookUrl"} type={"text"} placeholder="ChannelWebhookUrl" onChange={(e) => setUrl(e.target.value)}></Input>
        <div className="pt-4">
        <PrimaryButton onClick={() => {
            setMetadata({
                hookUrl
